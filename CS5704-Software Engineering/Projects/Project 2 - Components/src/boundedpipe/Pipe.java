...
public interface Pipe<E> extends Iterable<E> {
	...
	/**
	 * Returns a copy of this pipe. The elements in the copy are references to the
	 * elements in this pipe.
	 * 
	...
	/**
	 * Returns true if the specified object is equal to this pipe. To be equal, the
	 * specified object must be a bounded pipe with the same capacity, and it must
	 * contain the same number of elements with the same values in the same order.
	 * 
	 ...
	 * capacity. The list is enclosed in square brackets and adjacent elements are
	 * separated by a comma and a space. The list and capacity are separated by a
	 * colon. For example, the string <code>[A, B, C]:6</code> represents a pipe
	 * with 3 elements, where A is start element, C is the last element, and the
	 * pipe has a capacity of 6.
	 * 
